...
data "aws_ssm_parameters_by_path" "own_params" { 
  path      = "/teamcenter/${var.env_name}/enterprise_tier/corporate" 
  recursive = true
}
...
locals {
  ...
  exists_previous_rebake_seed = contains(data.aws_ssm_parameters_by_path.own_params.names, "/teamcenter/${var.env_name}/enterprise_tier/corporate/rebake_seed")
  index_previous_rebake_seed  = local.exists_previous_rebake_seed ? index(data.aws_ssm_parameters_by_path.own_params.names, "/teamcenter/${var.env_name}/enterprise_tier/corporate/rebake_seed") : -1
  ...
  is_different_than_fsc       = nonsensitive(data.aws_ssm_parameter.fsc_master_hostname.value) != nonsensitive(data.aws_ssm_parameter.corporate_hostname.value)
  is_start_fsc                = nonsensitive(contains(split(",", data.aws_ssm_parameter.fsc_all.value), data.aws_ssm_parameter.corporate_hostname.value))
  is_start_pool_mgr           = nonsensitive(contains(split(",", data.aws_ssm_parameter.svr_mgr_all.value), data.aws_ssm_parameter.corporate_hostname.value))
}
...
packer build \
  ...
  -var 'machine_name=${nonsensitive(data.aws_ssm_parameter.corporate_hostname.value)}' \
  ...
  -var 'stage_name=bake_corporate' \
  -var 'start_fsc=${local.is_start_fsc}' \
...
data "aws_ami" "corporate" {
  ...
  filter {
     name   = "tag:BuildUUID-tc-${nonsensitive(data.aws_ssm_parameter.corporate_hostname.value)}"
     values = [local.rebake_seed]
  }
}

resource "aws_ssm_parameter" "base_ami" {
  name           = "/teamcenter/${var.env_name}/enterprise_tier/corporate/base_ami"
  type           = "String"
  insecure_value = data.aws_ami.corporate.id
  overwrite      = true
}

resource "aws_ssm_parameter" "rebake_seed" {
  name           = "/teamcenter/${var.env_name}/enterprise_tier/corporate/rebake_seed"
  type           = "String"

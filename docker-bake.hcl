...
# ╰──────────────────────────────────────────────────────────╯
# docker buildx create --use --name "$(basename --suffix .git "$(git remote get-url origin)")" --driver docker-container
# ╭──────────────────────────────────────────────────────────╮
# │ 2-A run build without pushing to dockerhub               │
# ╰──────────────────────────────────────────────────────────╯
# LOCAL=true docker buildx bake --builder "$(basename --suffix .git "$(git remote get-url origin)")"
# ╭──────────────────────────────────────────────────────────╮
# │  2-B Run the build and push to docker hub                │
# ╰──────────────────────────────────────────────────────────╯
# docker buildx bake --builder "$(basename --suffix .git "$(git remote get-url origin)")"
# ╭──────────────────────────────────────────────────────────╮
# │                     cleanup builder                      │
# ╰──────────────────────────────────────────────────────────╯
...
#
# export IMAGE_NAME="$(basename --suffix .git "$(git remote get-url origin)")";
variable "IMAGE_NAME" {default="terraform-aws-snow-lz-ecs"}
# ────────────────────────────────────────────────────────────
...
#
# By default, the final 'basename --suffix .git' of the image is going to be `${var.IMAGE_NAME}:<target-name>`
#
...
#
# for instance we can include release version by setting `TAG` environment variable which would make the final image basename --suffix .git to be
...
#
# LOCAL=true docker buildx bake --builder "$(basename --suffix .git "$(git remote get-url origin)")" "devcontainer"
# ────────────────────────────────────────────────────────────
target "devcontainer" {
  context    = "."
  ...
  tags       = [
    ...
    : equal("",TAG)
      ? "${MAIN_REGISTRY_HOSTNAME}/${MAIN_REGISTRY_USERNAME}/${IMAGE_NAME}:archlinux"
      : "${MAIN_REGISTRY_HOSTNAME}/${MAIN_REGISTRY_USERNAME}/${IMAGE_NAME}:archlinux-${TAG}",
  ]
  cache-from = [
    equal(LOCAL,true)
    ? ""
    : equal("",TAG)
      ? "type=registry,mode=max,ref=${CACHE_REGISTRY_HOSTNAME}/${CACHE_REGISTRY_USERNAME}/${IMAGE_NAME}-cache:archlinux"
      : "type=registry,mode=max,ref=${CACHE_REGISTRY_HOSTNAME}/${CACHE_REGISTRY_USERNAME}/${IMAGE_NAME}-cache:archlinux-${TAG}"
  ]
  cache-to   = [
    equal(LOCAL,true)
    ? ""

...
variable "IMAGE_NAME" {default="kvctl"}
# ────────────────────────────────────────────────────────────
...
#
# LOCAL=true docker buildx bake --builder "$(basename "$(git remote get-url origin)")" "devcontainer"
# ────────────────────────────────────────────────────────────
target "devcontainer" {
  context    = "."
...
# ────────────────────────────────────────────────────────────
# This target builds Vault KVctl load runner image.
#
# ─── SNIPPETS ───────────────────────────────────────────────────────────────────
# ❯ Build the without pushing to a registry (export to local docker daemon)
#
# LOCAL=true docker buildx bake --builder "$(basename "$(git remote get-url origin)")" "kvctl"
target "kvctl" {
  context    = "."
  dockerfile = "contrib/docker/production/alpine/Dockerfile"
  tags       = [
    equal(LOCAL,true)
    ? "${IMAGE_NAME}:latest"
    : equal("",TAG)
  ...
  output     = [
    equal(LOCAL,true) ? "type=docker" : "type=registry"
  ]
}

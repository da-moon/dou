...
import (
	...
	git "github.com/DigitalOnUs/kvctl/internal/mage/git"
	primitives "github.com/DigitalOnUs/kvctl/internal/primitives"
...
const (
	packageName = "github.com/DigitalOnUs/kvctl"
)
...
func mkdir() error {
	...
	for _, v := range osMatrix {
		path := "bin"
		path = primitives.PathJoin(wd, path, v)
	...
	for _, v := range osMatrix {
		...
		for _, bin := range binaries {
			...
			args := []string{
				...
				"-o",
				primitives.PathJoin(wd, "bin", v, bin),
			...
			fmt.Fprintf(&buf, "  '")
			fmt.Fprintf(&buf, " -o %s", primitives.PathJoin(wd, "bin", v, bin))
			...
			binaryPath := primitives.PathJoin(wd, "bin", v, bin)
			strip, err := exec.LookPath("strip")
			if strip != "" && err == nil && v != "darwin" {

...
def initialize_swarm():
    ...
    """
    subprocess.check_call(["docker", "swarm", "init", "--default-addr-pool", "11.0.0.0/8"])
    ...
    subprocess.check_call(["sudo", "-u", "tc", "/home/tc/start_stack.sh"])
...
def get_manager_instance_vpc_tags(exclude_self=False):
    ...
    for vpc_instance in instances_considered:
        ...
        if "Role" in vpc_instance_tags and "ManagerJoinToken" in vpc_instance_tags:
...
def is_manager_role():
    current_instance_tags = instance_tags(get_current_instance())
    if "Role" in current_instance_tags:
        return current_instance_tags['Role'] == 'manager'
    return False 
...
#        subprocess.check_call(["yum", "install", "ec2-instance-connect"])
    if ssh_authorization_method == 'iam':

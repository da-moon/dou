...
resource "aws_subnet" "ms" {
  count                   = length(local.availability_zones)
  vpc_id                  = var.vpc_id
  cidr_block              = var.swarm_cidr_private_subnets == null ? cidrsubnet(data.aws_vpc.main.cidr_block, 8, var.ms_subnet_segment_start + count.index, ) : "${var.swarm_cidr_private_subnets[count.index]}"
  map_public_ip_on_launch = false

  tags = {
    Name = "${var.name}-ms-${count.index + 1}"
  }
...
resource "aws_route_table_association" "rtbl-association-managers" {
  count          = length(local.availability_zones)
  subnet_id      = aws_subnet.ms[count.index].id

...
data "aws_s3_objects" "maybe_self_outputs" {
  bucket = var.artifacts_bucket_name
  prefix = "stage_outputs/env-${var.env_name}/bake_aw_gateway/outputs.json"
}
...
locals {
  ...
  is_different_than_web_and_ms = nonsensitive(data.aws_ssm_parameter.web_hostname.value != data.aws_ssm_parameter.aw_gateway_hostname.value && data.aws_ssm_parameter.aw_gateway_hostname.value != data.aws_ssm_parameter.ms_hostname.value)
}
...
packer build \
  ...
  -var 'machine_name=${nonsensitive(data.aws_ssm_parameter.aw_gateway_hostname.value)}' \
  ...
  -var 'web_internal_dns=${nonsensitive(data.aws_ssm_parameter.aw_gateway_dns.value)}' \
...
resource "aws_s3_object" "webserver_outputs" {
  ...
  content = jsonencode({
    ami_id      = data.aws_ami.webserver_ami_id[0].id
    rebake_seed = local.rebake_seed
  })
}

...
func TestLocation(t *testing.T) {
	// New
	l := Location{20.643896, -103.416687}
	assert.NotNil(t, l)
	// Latitude, Longitude
...
func TestBoundaries(t *testing.T) {
	...
	minA, minB, maxA, maxB := GetBounds()
	assert.Equal(t, 0.0, minA)
	assert.Equal(t, 0.0, minB)
	assert.Equal(t, 1.0, maxA)
	assert.Equal(t, 1.0, maxB)
}

...
resource "aws_db_instance" "base_rds" {
  ...
  vpc_security_group_ids      = [aws_security_group.plm_dev.id]
...
resource "aws_db_subnet_group" "data" {
  name        = "${var.project_name}-db-${var.name}"
  description = "Database subnet group"
  subnet_ids  = [aws_subnet.plm_private[0].id, aws_subnet.plm_private[1].id]
}
...
resource "aws_security_group" "db_access" {
  name        = "${var.name}-${var.engine_type}-access"

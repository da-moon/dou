...
# terraform13 workspace new $TF_WORKSPACE 2> /dev/null
# This should be set using the TF_WORKSPACE environment variable
# echo "Current workspace: $(terraform13 workspace show)"

# Target
if [ -n "$TF_TARGET" ]; then  
    echo "##[command]terraform13 apply with -target"
    terraform13 apply -auto-approve -no-color $TF_TARGET
    status=$?
else
    echo "##[command]terraform13 apply with -var-file"
    terraform13 apply -auto-approve -no-color
    status=$?
fi    

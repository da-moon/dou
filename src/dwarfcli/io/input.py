...
from dwarfcli.config.style.pygments.lexer import FigLexer, DwarfPygment
...
class Input:
    """
    Contains some generic prompts that are used by multiple different executions paths indwarfgy
    """
    ...
    def select_region() -> str:
        ...
        while region not in AWS_REGIONS:
            region = prompt('Please input the AWS region to associatedwarfgy with: ',
    ...
    def select_auto_mfa() -> bool:
        return Input.y_n_input('Would you like to save your MFA secret to your keychain and have Dwarf generate '
    ...
    def select_report_errors() -> bool:
        selection = ''
        while selection.lower() != 'y' and selection.lower() != 'n':
            selection = input(f'Help make Dwarf better. turn on anonymous error reporting?  Y/n?: ')
            selection = selection.lower() if selection != '' else 'y'
        return selection == 'y'
    ...
    def select_usage_tracking() -> bool:
        selection = ''
        while selection.lower() != 'y' and selection.lower() != 'n':
            selection = input(f'Help make Dwarf better. Turn on 100% anonymous usage reporting Y/n?: ')
            selection = selection.lower() if selection != '' else 'y'
        return selection == 'y'
    ...
    def get_mfa(display_hint: bool = False, color: Optional[Color] = None) -> str:
        if display_hint and random.randint(0, 10) == 10:
            ...
            print(f"{blue}Hint:{rs} Tired of typing in your MFA? Consider saving your MFA secret to your keychain and "
                  ...
                  f"{blue}More info:{rs} http://dwarf.dev/docs/getting-started/install.html\n\n")

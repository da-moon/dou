...
from dwarfcli.io.output import Output
...
class Utils:
    ...
    @staticmethod
    def find_dwarf_json():
        for path in DEFAULT_DWARF_JSON_PATHS:
    ...
    def get_ci_config(self, ci_config_path: str) -> Dict:
        ...
        try:
            ...
            print(f"{self.c.fg_rd}Error decoding json in dwarf.json. Invalid JSON detected. "
    ...
    def get_namespace(self, config: Dict):
        if OPTIONAL_NAMESPACE in config:
            ...
            self.validate(CONFIG_KEY in config, f"You must specify an {CONFIG_KEY} or {OPTIONAL_NAMESPACE} block, "
                                                f"or both, in your dwarf.json file.")
    ...
    def parse_namespace(app_key: str) -> str:
        ...
        try:
            ...
            print(f"Unable to parse namespace from {app_key}. If your app_figs block values do not begin with "
                  f"the prefix /app/your-service-name , you must include the 'namespace' property in your dwarf.json "
    ...
    def format_config(config: Dict) -> OrderedDict:
        """
        Takes a formatted dwarf.json file dictionary and converts it to an ordered dictionary. This makes it possible
